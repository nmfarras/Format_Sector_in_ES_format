...
print(arc_start.latitude, arc_start.longitude)
...
boundary_points = (
    ...
    arc_points_tuples +
    [(point.longitude, point.latitude) for point in [arc_end, fixed_points[0]]]
)
...
print(f"  Bearing Start: {util.initial_bearing_angle(arp_sultan_thaha.latitude, arp_sultan_thaha.longitude, arc_end.latitude, arc_end.longitude)}")
print(f"  Bearing End: {util.initial_bearing_angle(arp_sultan_thaha.latitude, arp_sultan_thaha.longitude, arc_start.latitude, arc_start.longitude)}")
...
with open(f"{case_name}_points.txt", "w") as f:
    ...
    f.write("\n# ARP Sultan Thaha\n")
    f.write(f"{arp_sultan_thaha.longitude}, {arp_sultan_thaha.latitude}\n")
...
plt.scatter(arp_sultan_thaha.longitude, arp_sultan_thaha.latitude, color='red', label='ARP Sultan Thaha', s=100)
